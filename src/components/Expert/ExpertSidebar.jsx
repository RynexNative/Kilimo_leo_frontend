...
import { FaTachometerAlt, FaBookOpen, FaLeaf, FaCloudSun, FaChartLine, FaSignOutAlt, FaFirstdraft, FaList, FaLowVision, FaInvision, FaDiscourse, FaFacebookMessenger, FaTextHeight, FaVoicemail, FaSms, FaUsers, FaBook, FaBell, FaChartBar, FaCog  } from 'react-icons/fa';
...
const ExpertSidebar = () => {
  ...
  if(role == 'Expert'){
    ...
    return (
      <div className={styles.sidebar}>
        <h2 className={styles.logo}>Extension Officer</h2>
        <nav className={styles.nav}>
          ...
          <NavLink to="/extofficer/farmer-list" className={({ isActive }) =>
                  `${styles.link} ${isActive ? styles.active : ''}`}>
            <FaList /> <span>Farmer List</span>
          </NavLink>
          <NavLink to="/extofficer/weather-alert" className={({ isActive }) =>
                  `${styles.link} ${isActive ? styles.active : ''}`}>
            <FaCloudSun /> <span>Weather Alert</span>
          </NavLink>
